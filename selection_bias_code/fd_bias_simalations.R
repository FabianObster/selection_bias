...
get_parameters <- function(df, group_df, block_fit, block_size = 1000, max_steps = 50, tol = 0.001, rate = 0.1){
  # Initialization
...
fit_mb_lam_2 <- function(iter, df){
  formula <- as.formula(y ~bols(V1, V2, lambda = 2)+
                          bols(V3, V4, lambda = 2) +
                          bols(V5, lambda = 2))
  sim_df <- df %>% dplyr::mutate(y = rnorm(dim(df)[1]))
  mb_model <- mboost(formula = formula, data = sim_df, control = boost_control(mstop = 1))
...
run_experiment <- function(iteration){
  ...
  gr_lasso_005_sc_coef <- gr_lasso_005_sc$beta %>%
    ...
    dplyr::filter(coef != 0) %>%
    dplyr::slice(1:5)  %>%
    #dplyr::filter(lambda == gr_lasso_005_sc$lambda[1]) %>%
    dplyr::mutate(sel = case_when(rowname %in% c('V1','V2')~ 1,
                           rowname %in% c('V3','V4')~2,
                           rowname %in% c('V5')~ 3))
  ...
  gr_lasso_01_coef <- gr_lasso_01_coef %>%
    #dplyr::filter(lambda == gr_lasso_01_coef$lambda[1]) %>%
    dplyr::mutate(sel = case_when(rowname %in% c('V1','V2')~ 1,
                           rowname %in% c('V3','V4')~2,
                           rowname %in% c('V5')~ 3))
  ...
  gr_lasso_005_coef <- gr_lasso_005_coef %>%
    #dplyr::filter(lambda == gr_lasso_005_coef$lambda[1]) %>%
    dplyr::mutate(sel = case_when(rowname %in% c('V1','V2')~ 1,
                           rowname %in% c('V3','V4')~2,
                           rowname %in% c('V5')~ 3))
  ...
  #                       gr_lasso_coef$rowname %in% c('V3', 'V4') ~ 2,
  #                       rowname %in% c('V5')~ 3)
  # }
  ret <- ret %>% 
...
run_experiment <- function(iteration, intercept = TRUE, scaled = TRUE, type = 'ortho'){
  ...
  mb_model_unreg <- mboost::mboost(y ~bols(V1)+bols(V2, V3, V4,V5, V6, V7,V8, V9, V10,V11, V12, V13,V14,V15),
                           data = dat, control = boost_control(mstop = 1, nu = 1))
  mb_model_0.5 <- mboost::mboost(y ~bols(V1,df =0.5)+bols(V2, V3, V4,V5, V6, V7,V8, V9, V10,V11, V12, V13,V14,V15, df=0.5),
                         data = dat, control = boost_control(mstop = 1, nu = 1))
  mb_model_0.1 <- mboost::mboost(y ~bols(V1,df =0.1)+bols(V2, V3, V4,V5, V6, V7,V8, V9, V10,V11, V12, V13,V14,V15, df=0.1),
                         data = dat, control = boost_control(mstop = 1, nu = 1))
  mb_model_1 <- mboost::mboost(y ~bols(V1,df =1)+bols(V2, V3, V4,V5, V6, V7,V8, V9, V10,V11, V12, V13,V14,V15, df=1),
                       data = dat, control = boost_control(mstop = 1, nu = 1))
  mb_model_lam <- mboost::mboost(y ~bols(V1,lambda =10)+bols(V2, V3, V4,V5, V6, V7,V8, V9, V10,V11, V12, V13,V14,V15, lambda=10),
                         data = dat, control = boost_control(mstop = 1, nu = 1))
  gr_lasso <- gglasso(x = as.matrix(dat %>% select(-y)), y = dat$y,
                      group = c(1,rep(2,15)))
  gr_lasso_coef <- gr_lasso$beta %>% 
...
results_df %>%
  dplyr::group_by(scaled, model, type, sel) %>%
  dplyr::tally() %>%
  dplyr::filter(!is.na(sel)) %>%
  dplyr::ungroup() %>%
...
dat <- data.frame(V1 = factor(sample(LETTERS[1:3], 120, replace=TRUE, prob=c(0.7, 0.15,0.15))), 
                  ...
                  V4 = rnorm(120))
run_experiment <- function(iteration, intercept = TRUE){
  dat <- dat %>% mutate(y = rnorm(120))
  dat_model <- model.matrix(~., select(dat,-y))[,-1]
  forward_model <- lm(y ~ 1, data = dat)
  forward_model <- add1(forward_model, direction = "forward", scope = y~V1+V2+V3+V4, test = 'F')
  forward_aic_sel <- forward_model %>% as.data.frame() %>% rownames_to_column() %>% slice(-1) %>% filter(AIC == min(AIC))
  forward_aic_sel <- forward_aic_sel$rowname %>% substr(2,2) %>% as.numeric()
  forward_F_sel <- forward_model %>% as.data.frame() %>% rownames_to_column() %>% slice(-1)
  forward_F_sel <- forward_F_sel[forward_F_sel$`Pr(>F)` == min(forward_F_sel$`Pr(>F)`),]$rowname %>% substr(2,2) %>% as.numeric()
  if(length(forward_F_sel) == 0){forward_F_sel <- NA}
  ...
  ret <- data.frame(model = c('mb', 'mb_0.5', 'mb_1', 'mb_0.1', 'mb_lambda', 'grla', 'forward_aic', 'forward_F'), 
                    sel = c(mb_model_unreg$xselect(), mb_model_0.5$xselect(),
                            mb_model_1$xselect(), mb_model_0.1$xselect(),
                            mb_model_lam$xselect(), gr_sel, forward_aic_sel, forward_F_sel))
  return(ret)
}
library(furrr)
plan(multisession)
...
results_df <- params %>%
  ...
  unnest(cols = res)
tictoc::toc()
saveRDS(file = 'results_fixed/small_cat_1_raw.RDS', results_df)
results_df %>%
  dplyr::group_by(model, sel) %>%
  dplyr::tally() %>%
  dplyr::filter(!is.na(sel)) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(model) %>%
  dplyr::mutate(prop = round(n/sum(n,na.rm = T), 3)) %>%
  saveRDS(file = 'results_fixed/small_cat_1.RDS')
plan(sequential)
...
################################################################################
###                      Analysis                                            ###
res_df <- readRDS(file = 'results_fixed/small_cat_1.RDS', results_df)
res_df <- readRDS(file = 'results_fixed/small_group_1.RDS', results_df)
res_df <- readRDS(file = 'results_fixed/big_group_1.RDS', results_df)
